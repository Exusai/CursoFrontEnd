...
const pokeAtk = document.querySelector('.poke-atk');
      ...
      for (let i = 0; i < pokeListItems.length ; i++) {
        ...
        if (resultData) {
          ...
          .then(pokemonInfo => {
            ...
            console.log(image)
            pokeListItem.style.backgroundImage = "url(" + image +")";
          });
        } else {
          pokeListItem.textContent = '';
          //pokeListItem.style.backgroundImage = null;
        }
      }
    });
};
...
const fetchPokeData = id => {
  fetch(`https://pokeapi.co/api/v2/pokemon/${id}`)
    ...
    .then(data => {
      ...
      pokeMoves.textContent = data['moves'].length;    
